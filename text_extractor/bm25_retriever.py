...
def prepare_documents(documents, transcriptions, captions):
    """Prepare documents for BM25 retrieval."""
    doc_texts = []
    doc_ids = []
    for idx, doc in enumerate(documents):
        audio_id = doc["audio_id"]
        doc_transcription = transcriptions[audio_id]
        doc_caption = captions[f"{idx:05d}"]
...
def process_split(
    ...
    queries_transcriptions_dir = os.path.join(transcriptions_dir, "queries", split)
    queries_captions_dir = os.path.join(captions_dir, "queries", split)
    documents_transcriptions_dir = os.path.join(transcriptions_dir, "documents", split)
    documents_captions_dir = os.path.join(captions_dir, "documents", split)

    queries_transcriptions = load_text_files(queries_transcriptions_dir)
    queries_captions = load_text_files(queries_captions_dir)
    documents_transcriptions = load_text_files(documents_transcriptions_dir)
    documents_captions = load_text_files(documents_captions_dir)

    doc_texts, doc_ids = prepare_documents(
        documents[split], documents_transcriptions, documents_captions
    )
    ...
    with open(output_file, "w") as f:
        ...
        for idx, query in enumerate(bar):
            ...
            query_transcription = queries_transcriptions[audio_id]
            query_caption = queries_captions[f"{idx:05d}"]
