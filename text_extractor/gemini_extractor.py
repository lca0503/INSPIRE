...
def get_embedding(text, client, model_name, task_type):
    text = text.replace("\n", " ")
    ...
    embeddings = results.embeddings[0].values
...
def extract_queries_rep(
    ...
    for idx, query in enumerate(tqdm(queries, desc="Extract queries representations")):
        ...
        embeddings = get_embedding(input_text, client, model_name, "RETRIEVAL_QUERY")
...
def extract_documents_rep(
    ...
    for idx, document in enumerate(
        ...
        embeddings = get_embedding(input_text, client, model_name, "RETRIEVAL_DOCUMENT")
